...
class TestLoggingSetup:
    ...
    def test_setup_logging_console_only(self, caplog):
        ...
        setup_logging(
            level=logging.INFO, console_output=True, file_output=None
        )
    ...
    def test_setup_logging_with_file_output(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            setup_logging(
                level=logging.INFO, console_output=False, file_output=log_file
            )
...
class TestLoggingHelpers:
    ...
    def test_log_data_info(self, caplog):
        ...
        assert (
            'Loaded ASL image: shape=(64, 64, 30, 100), path=/path/to/file.nii'
            in caplog.text
        )
    ...
    def test_log_warning_with_context(self, caplog):
        ...
        assert (
            'Something went wrong (Context: during registration)'
            in caplog.text
        )
...
def test_logging_level_configuration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Test DEBUG level
        setup_logging(
            level=logging.DEBUG, console_output=False, file_output=log_file
        )
        logger = get_logger()
        ...
        # Test WARNING level
        setup_logging(
            level=logging.WARNING, console_output=False, file_output=log_file
        )
        logger = get_logger()
