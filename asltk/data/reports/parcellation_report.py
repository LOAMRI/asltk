...
class ParcellationReport(BasicReport):
    def __init__(self, subject_image: ASLData, atlas_name: str = 'MNI2009',
        ...
        self.subject_filename = subject_filename if subject_filename else 'Unknown'
        self.subject_img_dimensions = subject_img_dimensions if subject_img_dimensions else (0, 0, 0)
        self.subject_img_type = subject_img_type if subject_img_type else 'Unknown'
        self.subject_img_resolution = subject_img_resolution if subject_img_resolution else (0, 0, 0)
        ...
        self.default_fullpath = os.path.join(default_path, self.report_filename)
    ...
    def save_report(self, format: str = 'csv'):
        ...
        if not self.report_data:
            raise ValueError("Report data is empty. Please generate the report first.")
        ...
        if format == 'csv':
            ...
            self.report_data['regions_dataframe'].to_csv(file_path, index=False)
        elif format == 'pdf':
    ...
    def _create_description_section(self):
        ...
        ax1.axis('off')
        ax1.text(0.01, 0.5, f"Parcellation Report", fontsize=16, fontweight='bold')
        ax1.text(0.01, 0.1, f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M')}", fontsize=10)
        
        # Brain Atlas: Name and description
        ax2 = plt.subplot(gs[1])
        ax2.axis('off')
        ax2.text(0.01, 0.7, f"Brain Atlas Information", fontsize=14, fontweight='bold')
        ax2.text(0.01, 0.4, f"Name: {self.atlas.name}")
        ax2.text(0.01, 0.1, f"Description: {getattr(self.atlas, 'description', 'No description available')}")
        
        # Subject Information
        ax3 = plt.subplot(gs[2])
        ax3.axis('off')
        ax3.text(0.01, 0.9, "Subject Information", fontsize=14, fontweight='bold')
        ...
        ax3.text(0.01, 0.1, f"Image resolution: {self.subject_img_resolution} mm")
        ...
        try:
            ...
            ax4.axis('off')
            ax4.text(0.01, 0.95, "Brain Regions", fontsize=14, fontweight='bold')
            ...
            ax4 = plt.subplot(gs[3])
            ax4.axis('off')
            ax4.text(0.01, 0.5, f"Brain Regions: Error retrieving region information. {str(e)}", 
        ...
        return {
            ...
            'atlas_description': self.atlas.get('description', 'No description available'),
            'subject_info': {
    ...
    def _check_inputs_dimensions(subject_image, atlas):
        ...
        if subj_dim != atlas_dim:
            raise TypeError(f'subject_image must have the same dimensions as the atlas image. Dimensions do not match: {subj_dim} != {atlas_dim}')
