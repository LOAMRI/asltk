...
from asltk.logging_config import get_logger, log_processing_step, log_warning_with_context
...
def head_movement_correction(
    ...
    if not isinstance(ref_vol, int) or ref_vol >= len(total_vols):
        ...
        logger.error(f"{error_msg} ref_vol={ref_vol}, total_volumes={len(total_vols)}")
        raise ValueError(error_msg)

    # Apply the rigid body registration to each volume (considering the ref_vol)
    log_processing_step("Applying rigid body registration", f"using volume {ref_vol} as reference")
    corrected_vols = []
    trans_mtx = []
    ...
    logger.info(f"Head movement correction completed successfully for {len(total_vols)} volumes")
